...
## An impulse force amount that is applied when the slam makes a direct hit with another player
@export var slam_direct_hit_force = 50.0
## A shape used in the shapeCast calculation to determine the area which players are affected by a slam
@export var slam_impact_shape: Shape3D
## The amount of time a player must wait after leaving the ground before being allowed to slam
@export var slam_allowed_delay: float = 0.5
...
func _slam_cast() -> void:
	var space_state = target_rigidbody.get_world_3d().direct_space_state
	var query = PhysicsShapeQueryParameters3D.new()
	query.shape = slam_impact_shape
	query.transform = target_rigidbody.transform
	var cast_results = space_state.intersect_shape(query)

	var momentum_y = highest_y_velocity_during_slam * target_rigidbody.mass

	effect_spawner.create_effect(target_rigidbody.global_position)

	for body in cast_results:
		var collider = body.collider as LaunchableRigidbody3D
		if (!collider is LaunchableRigidbody3D || collider == get_parent()):
			continue

		var body_direction = collider.global_position - target_rigidbody.global_position
		...
		if (collider.global_position.y < target_rigidbody.global_position.y):
			collider.launch(
				Vector3(
					...
					0,
					randf_range(-1, 1),
				).normalized() * slam_direct_hit_force,
				get_parent(),
				false,
			)
		else:
			collider.launch(body_direction * momentum_y * slam_launch_force_multiplier, get_parent(), false)
