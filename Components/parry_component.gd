...
func _create_parry_effect(spawn_direction: Vector3, look_at_position: Vector3, was_successful: bool = false) -> void:
	var parry_visuals = effect_spawner.create_effect(
		get_parent().global_position + parry_effect_offset + spawn_direction * parry_visual_spawn_distance,
	)

	parry_visuals.look_at(Vector3(look_at_position.x, parry_visuals.global_position.y, look_at_position.z))
	parry_visuals.scale = Vector3.ONE * parry_visual_scale_multiplier
...
func _on_parry_timer_timeout() -> void:
	if (!did_succesfully_parry):
		...
		var look_at_position = get_parent().global_position.direction_to(spawn_direction) * 1
		_create_parry_effect(spawn_direction, look_at_position)
		parry_failure.emit()
