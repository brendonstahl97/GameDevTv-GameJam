...
## base amount of time the bot is required to wait between code submissions on hard difficulty
@export var code_submission_base_cooldown_hard: float = 0.5
## how much the cooldown time can vary in both directions on hard difficulty
@export var code_submission_cooldown_variability_hard: float = 0.1
## percentage chance that the bot will submit an incorrect code on hard difficulty
@export_range(0, 100) var incorrect_submission_chance_hard: int = 10

@export_category("Difficulty Medium")
## base amount of time the bot is required to wait between code submissions on medium difficulty
@export var code_submission_base_cooldown_medium: float = 0.5
## how much the cooldown time can vary in both directions on medium difficulty
@export var code_submission_cooldown_variability_medium: float = 0.1
## percentage chance that the bot will submit an incorrect code on medium difficulty
@export_range(0, 100) var incorrect_submission_chance_medium: int = 10

@export_category("Difficulty Easy")
## base amount of time the bot is required to wait between code submissions on easy difficulty
@export var code_submission_base_cooldown_easy: float = 0.5
## how much the cooldown time can vary in both directions on easy difficulty
@export var code_submission_cooldown_variability_easy: float = 0.1
## percentage chance that the bot will submit an incorrect code on easy difficulty
@export_range(0, 100) var incorrect_submission_chance_easy: int = 10
...
func select_code_input() -> global.CodeDirection:
	...
	get_tree().create_timer(
		code_submission_base_cooldown + randf_range(
			-code_submission_cooldown_variability,
			code_submission_cooldown_variability,
		),
	).timeout.connect(_allow_submission)
...
func _get_random_incorrect_direction(correct_direction: global.CodeDirection) -> global.CodeDirection:
	var possible_directions := [
		global.CodeDirection.UP,
		global.CodeDirection.DOWN,
		global.CodeDirection.LEFT,
		global.CodeDirection.RIGHT,
	]
