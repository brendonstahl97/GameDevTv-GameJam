...
## Used to multiplicatively adjust the amount of stamina gained from bumping another player
@export var bump_stamina_gain_multiplier = 1.0
...
func _on_body_entered(body: Node3D):
	...
	if (target_bumpable_component.try_bump(get_parent())):
		var restored_stamina_amount = (
			get_parent().linear_velocity.length() * get_parent().mass * bump_stamina_gain_multiplier
		)
