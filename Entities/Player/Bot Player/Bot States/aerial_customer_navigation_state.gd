...
@export var slam_trigger_distance_threshold: float = 1.0 ## How close to the customer the bot needs to be to initiate a slam
...
func physics_update(_delta: float) -> void:
	...
	if (closest_customer == null):
		...
		var distance_to_player_no_y: float = Vector3(closest_customer.global_position.x, 0, closest_customer.global_position.z).distance_to(Vector3(bot_player.global_position.x, 0, bot_player.global_position.z)) 
