...
func enter() -> void:
	...
	# if there is not a customer, or there is not a player on the closest customer,
...
func update(delta: float) -> void:
	...
	if (
		...
		|| target_customer.currentPlayer == bot_player
		|| target_customer.currentPlayer == null
	):
		transitioned.emit(self, "FindClosestCustomerState")
		return
	...
	if (sprint_component.is_sprinting):
		if (!stamina_manager.try_drain_stamina(sprint_component.sprint_stamina_drain * delta)):
			bot_player.end_sprint()
	else:
		bot_player.begin_sprint()
